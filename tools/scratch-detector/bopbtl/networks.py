...
# sync_batchnorm (DataParallelWithCallback) removed: the original constructor rebinds a local `self` and has no effect; plain BatchNorm2d loads the same state dict.
...
class UNetUpBlock(nn.Module):
    ...
    def center_crop(self, layer, target_size):
        # Stereologue: with padding=True and input dims divisible by 16 the
        # bridge and the upsampled tensor always match, so skip the crop. This
        # keeps size() arithmetic out of the trace, which coremltools 9 cannot
        # convert for a static input shape.
        if tuple(layer.shape[2:]) == tuple(target_size):
            return layer
