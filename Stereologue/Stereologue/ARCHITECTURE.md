...
Converts NYPL card scans into spatial HEIC data (visionOS) and cropped
stereo pairs (flat-screen wiggle/anaglyph views). The shared pipeline
`preparedStereoPair(for:quality:style:)` produces the two eye images:

```
download (Nuke) → crop L/R → optional restoration → rectify → resize
```

The viewer path (`spatialHEICData`), the share path (`shareableSpatialPhotoURL`),
and the flat-screen path (`croppedStereoPair`) all build on that one pipeline.

Key invariants:
- **In-memory display, no disk round-trip.** `spatialHEICData(for:…)` encodes
  the spatial HEIC into a `Data` (via `CGImageDestinationCreateWithData`) and
  the viewer builds a `CGImageSource` straight from those bytes for
  `ImagePresentationComponent(imageSource:)`. Nothing is written to or read
  back from disk to display a photo.
- **Cache.** Generated bytes are held in a bounded in-memory LRU (`dataCache`,
  16 variants) keyed by `<uuid>_<quality>[_restored_<style>]`. Session-only:
  the source download is still Nuke-disk-cached, so regeneration after relaunch
  is cheap.
- **Coalescing.** Concurrent calls for the same variant are merged into one
  in-flight `Task<Data, Error>` via `inFlightTasks: [String: Task<...>]`.
...
- **Shareable variant.** `shareableSpatialPhotoURL(for:metadata:)` is the only
  path that touches the filesystem — it regenerates every time, embeds
  IPTC/TIFF metadata (title, creator, places, etc.), and writes a file to
  `~/Library/Caches/Stereologue/SpatialPhotos/` on demand for the share sheet.
...
the main actor. Backs the mosaic tiles in the Subjects/Creators/Places/
Collections lists (`BrowseMosaicGrid` / `BrowseMosaicItem`).
...
3. **`actor`** — used for `SpatialPhotoService`, `RestorationPipeline`,
   `StereoRectificationService`, and `CatalogQueryService` (a `@ModelActor`
   owning a background `ModelContext`). Each owns serial state (caches,
   in-flight task maps, `CIContext`s, a scratch context) and exposes async
   methods.
